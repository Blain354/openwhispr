...
| Secrets    | a custom LLM key goes to the sidecar in `OW_CONVERSATION_LLM_API_KEY`, never over the socket                                                                    |
...
4. Sidecar loads Whisper (CUDA) and Kokoro, warms both, starts the pipeline, and sends `ready`.
